...
static int checkItemToggle(PlayState* play)
{
    ...
    itemId == ITEM_OOT_SHIELD_DEKU)
    {
        ...
        if (popcount(owned) > 1)
        {
                ...
                U_CBUTTONS)
            {
                ...
                next =
    OotChildShield_GetNextOwned(OotChildShield_GetVariant());
    ...
    if (p->pageIndex == PAUSE_EQUIP &&
    p->cursorSpecialPos == 0 &&
    itemId == ITEM_OOT_SWORD_KOKIRI &&
    gSharedCustomSave.extraSwordsOot > 0)
    {
        ret = 1;
    ...
    itemId <= ITEM_OOT_SHIELD_MIRROR &&
    gSave.info.equips.equipment.shields ==
        ((itemId - ITEM_OOT_SHIELD_DEKU) + 1))
    {
        ret = 1;
    ...
    {
        if (!(GET_PLAYER(play)->stateFlags2 &
              PLAYER_ACTOR_STATE_WATER))
        {
            ret = 1;
...
static void KaleidoScope_UpdateEquipVariantOutlines(
    PlayState* play)
{
    PauseContext* pauseCtx;
    u8 selected;
        ...
        gSharedCustomSave.extraSwordsOot > 0)
    {
        ...
        KaleidoScope_SetNativeVariantOutline(
            play,
            OOT_EQUIP_QUAD_SWORD_KOKIRI,
            OOT_EQUIP_QUAD_SELECTED_SWORD,
            selected == equipped);
    }
    owned = gSharedCustomSave.ootChildShields;
        ...
        popcount(owned) > 1)
    {
        ...
        KaleidoScope_SetNativeVariantOutline(
            play,
            OOT_EQUIP_QUAD_SHIELD_DEKU,
            OOT_EQUIP_QUAD_SELECTED_SHIELD,
            selected == equipped);
    }
}
...
void KaleidoScope_LoadItemName(void* dst, s16 id)
{
    ...
    else if (itemId == ITEM_OOT_SHIELD_DEKU &&
         OotChildShield_GetVariant() == OOT_CHILD_SHIELD_HERO)
    {
        comboLoadMmIcon(
            dst,
            0xa27660,
...
static Vtx* KaleidoScope_GetEquipVariantVtx(
    PlayState* play,
    s32 equipType)
{
    const Vtx* src;
    ...
    s32 quad;
    s32 i;
    quad = equipType == OOT_EQUIP_VARIANT_SWORD ? 1 : 5;
...
static Vtx* KaleidoScope_GetEquipVariantOutlineVtx(
    PlayState* play,
    s32 equipType)
{
    Vtx* src;
    ...
    src = KaleidoScope_GetEquipVariantVtx(play, equipType);

    frame = play->state.gfxCtx->displayListCounter & 1;
    dst = &sEquipVariantOutlineVtx[equipType][frame * 4];
...
static void KaleidoScope_DrawEquipVariantOutline(
...
void KaleidoScope_DrawQuadEquipment(GraphicsContext* gfxCtx, u32 dlist, int w, int h, int flags)
{
    ...
    static u8 customKokiSwordAge = 0xff;
    ...
    if (originalDlist == 0x0803b000)
    {
        ...
        if (sword != 0)
        {
            ...
            if (customKokiriSwordTexture)
            {
                if (customKokiriSwordId != sword || customKokiSwordAge != gSave.age)
                {
            ...
            else
            {
                ...
                if (customKokiriSwordSmallTexture)
                {
                    if (customKokiriSwordSmallId != next || customKokiriSwordSmallAge != gSave.age)
                    {
                        ...
                        {
                        case 1:
                            LoadMmItemIcon(customKokiriSwordSmallTexture, ITEM_MM_SWORD_RAZOR);
                            break;
                        case 2:
                            LoadMmItemIcon(customKokiriSwordSmallTexture, ITEM_MM_SWORD_GILDED);
                            break;
                        }

                        if (gSave.age != AGE_CHILD && !Config_Flag(CFG_OOT_AGELESS_SWORDS))
                            Grayscale(customKokiriSwordSmallTexture, 0x400);
                    }
    ...
    if (originalDlist == 0x0803e000)
    {
        ...
        s32 hasSecondary;
        s32 needHeroTexture;
        shield = OotChildShield_GetVariant();
        owned = gSharedCustomSave.ootChildShields;
        next = shield == OOT_CHILD_SHIELD_DEKU ? OOT_CHILD_SHIELD_HERO : OOT_CHILD_SHIELD_DEKU;
        hasSecondary = popcount(owned) > 1 && (owned & (1u << next));
        if (hasSecondary && gSave.info.equips.equipment.shields == 1 && OotChildShield_GetEquippedVariant() == next)
        {
            secondaryEquipped = 1;
        }
        needHeroTexture = shield == OOT_CHILD_SHIELD_HERO || (hasSecondary && next == OOT_CHILD_SHIELD_HERO);
