...
const mmChildEffectBase = (numSfx: number) => alignUp(numSfx, 0x40);
const mmAdultEffectBase = (numSfx: number) => mmChildEffectBase(numSfx) + 0x40;
...
const buildMmAdultVoiceHybridBank = async (roms: DecompressedRoms, mmBankTable: Uint8Array) => {
    ...
    const childEffectBase = mmChildEffectBase(mm.numSfx);
...
function patchMmAdultVoiceSequence(
    ...
    const childEffectBlock = childEffectBase >>> 6;
    ...
    const childPointerTable = new Uint8Array(
        seq.subarray(
            MM_HUMAN_VOICE_TABLE_OFFSET,
            MM_HUMAN_VOICE_TABLE_OFFSET + MM_HUMAN_VOICE_TABLE_SIZE
        )
    );

    if (childPointerTable.length !== MM_HUMAN_VOICE_TABLE_SIZE)
    ...
    const adultPointerTableOffset = childPointerTableOffset + MM_HUMAN_VOICE_TABLE_SIZE;
    const data = concatUint8Arrays([seq, body, childPointerTable, adultPointerTable]);

    data.set(childPointerTable, MM_HUMAN_VOICE_TABLE_OFFSET);

    return {
        data,
        humanPointerTableOffset: MM_HUMAN_VOICE_TABLE_OFFSET,
    ...
    async extractSeqTable(game: Game, count: number, codeOffset: number, romOffset: number) {
        ...
        for (let i = 0; i < count; ++i) {
            ...
            if (game === 'mm' && i === 0) {
                ...
                this.cg.define('CUSTOM_MM_HUMAN_VOICE_POINTER_TABLE_OFFSET', patched.humanPointerTableOffset);
                this.cg.define('CUSTOM_MM_CHILD_VOICE_POINTER_TABLE_OFFSET', patched.childPointerTableOffset);
                this.cg.define('CUSTOM_MM_ADULT_VOICE_POINTER_TABLE_OFFSET', patched.adultPointerTableOffset);
            } else
                addr += romOffset;
