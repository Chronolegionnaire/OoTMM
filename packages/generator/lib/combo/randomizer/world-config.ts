...
export function worldConfig(world: LogicResultWorld, settings: Settings): Set<Confvar> {
  ...
  const exprs: { [k in Confvar]: boolean } = {
    ...
    OOT_PROGRESSIVE_BIGGORON_SWORD: settings.progressiveSwordsOot !== 'separate',
    MM_PROGRESSIVE_BIGGORON_SWORD: settings.biggoronSwordMm === 'progressive' || (settings.biggoronSwordMm === 'shared' && settings.progressiveSwordsOot !== 'separate'),
    MM_MASTER_SWORD: settings.masterSwordMm,
    MM_BIGGORON_SWORD: settings.biggoronSwordMm !== 'none',
    SHARED_CHILD_SWORDS: settings.sharedChildSwords,
    SHARED_MASTER_SWORD: settings.sharedMasterSword,
    SHARED_BIGGORON_SWORD: settings.biggoronSwordMm === 'shared',
