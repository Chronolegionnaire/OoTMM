...
const OOT_OBJECT_TABLE_OFFSET = 0xe7f58;
const MM_OBJECT_TABLE_OFFSET = 0x11cc80;
const GAMEPLAY_KEEP_OBJECT_ID = 0x01;
...
function runtimeGameplayKeep(builder: RomBuilder, game: Game) {
  const original = builder.fileByNameRequired(`${game}/objects/gameplay_keep`);
  const code = builder.fileByNameRequired(`${game}/code`).data;
  const tableOffset = game === 'oot' ? OOT_OBJECT_TABLE_OFFSET : MM_OBJECT_TABLE_OFFSET;
  const entryOffset = tableOffset + GAMEPLAY_KEEP_OBJECT_ID * 8;
  if (entryOffset + 8 > code.length)
    throw new Error(`${game}: gameplay_keep object table entry out of range`);

  const start = bufReadU32BE(code, entryOffset);
  const end = bufReadU32BE(code, entryOffset + 4);
  if (!start || end <= start || start === original.vaddr) return original.data;

  const found = findFileAtVrom(builder, start, end - start);
  if (!found)
    throw new Error(`${game}: relocated gameplay_keep VROM 0x${start.toString(16)} not found`);
  return found.file.data.subarray(found.offset, found.offset + end - start);
}

export function applyPlayerAnimationDedupe(builder: RomBuilder, plan: PlayerAnimationDedupePlan) {
  patchHeaders(runtimeGameplayKeep(builder, 'oot'), plan.ootHeaderPointers, 'OoT');
  patchHeaders(runtimeGameplayKeep(builder, 'mm'), plan.mmHeaderPointers, 'MM');
...
function collectLocalSamples(game: Game, builder: RomBuilder, roms: DecompressedRoms): LocalCollection {
  ...
  const runtimeSamples = parseAudioEntries(
      builder.fileByNameRequired(`${game}/audio_table`).data,
      0,
      Math.floor(builder.fileByNameRequired(`${game}/audio_table`).data.length / AUDIO_ENTRY_SIZE),
  );
...
function collectBlobs(entries: AudioEntry[], file: Uint8Array, protectedId = 0, live?: Set<string>): Blob[] {
...
function liveNativeBlobKeys(
    builder: RomBuilder,
    game: Game,
    filename: 'Audioseq' | 'Audiobank',
    runtimeTable: 'seq_table' | 'bank_table',
    sourceEntries: AudioEntry[],
) {
  const file = builder.fileByNameRequired(`${game}/${filename}`);
  if (file.legacyPaddr === undefined) return undefined;

  const runtime = builder.fileByNameRequired(`${game}/${runtimeTable}`).data;
  const runtimeEntries = parseAudioEntries(runtime, 0, Math.floor(runtime.length / AUDIO_ENTRY_SIZE));
  const out = new Set<string>();
  const count = Math.min(sourceEntries.length, runtimeEntries.length);

  for (let id = 0; id < count; ++id) {
    const resolved = resolveAudioEntry(runtimeEntries, id).entry;
    if (!resolved.size || resolved.romAddr >= 0x08000000) continue;
    if (resolved.romAddr < file.legacyPaddr) continue;

    const offset = resolved.romAddr - file.legacyPaddr;
    if (offset + resolved.size > file.data.length) continue;
    out.add(audioBlobKey(offset, resolved.size));
  }
...
function buildFilePlan(roms: DecompressedRoms, builder: RomBuilder, kind: 'seq' | 'bank'): FilePlan {
  ...
  const ootFile = builder.fileByNameRequired(`oot/${filename}`);
  const mmFile = builder.fileByNameRequired(`mm/${filename}`);
  const ootLive = liveNativeBlobKeys(builder, 'oot', filename, runtimeTable, ootEntries);
  const mmLive = liveNativeBlobKeys(builder, 'mm', filename, runtimeTable, mmEntries);

  const ootCanonical = new Map<string, { data: Uint8Array; offset: number }[]>();
  for (const blob of collectBlobs(ootEntries, ootFile.data, 0, ootLive)) {
  ...
  for (const blob of collectBlobs(mmEntries, mmFile.data, 0, mmLive)) {
  ...
  return {
    ...
    mmEntries,
    mmMap,
    mmLegacyBase: mmFile.legacyPaddr,
  };
}
...
function finalizePlan(builder: RomBuilder, plan: FilePlan) {
  ...
  const oldBase = plan.mmLegacyBase ?? deriveOldPhysicalBase(runtime, plan.mmEntries);
