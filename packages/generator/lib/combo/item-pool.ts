...
export async function itemPool(settings: Settings): Promise<Items> {
  ...
  for (const pi of pool.keys()) {
    if (
        pi.player !== 0 ||
        (itemProperties.junk.has(pi.item) && !ItemHelpers.isItemUnlimitedStarting(pi.item)) ||
        ItemHelpers.isDungeonItem(pi.item) ||
        ItemHelpers.isToken(pi.item) ||
        ItemHelpers.isStrayFairy(pi.item)
    ) {
      pool.delete(pi);
